...
- **方式 4**：在下方支持的千帆宽表页面点扩展图标 → 「开启宽屏分析」；需要回到原页面时点「恢复官方布局」

### 千帆宽屏分析说明

- 只作用于当前千帆标签页，整页刷新后自动回到小红书官方布局
- 仅匹配 15 条精确网址，不会把宽屏样式扩散到订单查询、发货、售后或其他千帆页面
- 商品笔记继续保留笔记信息、商品、发布时间和操作列的独立宽度；指标列最低 84px，表头允许两行展示，字号保持小红书原始 14px
- 其余页面保持小红书官方列宽和字号，只把整个工作区扩到足以容纳当前表格；屏幕不足时保留横向滚动，不强行压缩
- 左侧菜单和主内容作为一个整体居中；买手广场等超宽表会按真实 `scrollWidth`（表格完整宽度）自动多扩一档
- 如果屏幕本身不足以容纳可读列宽，会保留横向滚动，不以牺牲可读性换取“强行同屏”
- 页面筛选、排序、翻页和「笔记详情」继续使用小红书原有功能
- 如果页面表格尚未加载或当前筛选结果为空，插件会先进入等待状态，表格出现后自动应用，不会出现空白页
- 点击「恢复官方布局」会还原左侧菜单位置、页面外壳边距、宽度、滚动方式和所有表格原始样式

#### 已纳入的精确页面

| 区域 | 页面 |
|---|---|
| 数据中心 | 商品笔记、商品总览、成交分析、搜索总览、笔记蓝链、退款分析、账号分析、直播场次、实时商品、引流搜索词、买手分析 |
| 千帆主后台 | 售卖中商品、营销数据、商品合作、买手广场 |

订单查询、发货管理、售后工作台、物流异常、流量来源等已确认无需加宽的页面不在白名单中。

#### v4.10.1 实机验收结果

- 15/15 个页面在 2601px 宽屏视口通过：目标表格横向溢出均为 0，左右居中误差均为 0px。
- 所有目标表头字号开启前后均为 14px；筛选、排序、分页和固定操作列功能仍存在。
- 每个页面关闭宽屏后，菜单位置、工作区宽度、表格宽度和标记均恢复到开启前状态。
- 商品管理、营销和分销页面的内部锁宽容器已单独兼容；千帆异步刷新后也会保持居中。
- 实机截图保存在 `/Users/a86132/Desktop/codex开发/qianfan-v4.10-e2e/`，用于逐页复核。
